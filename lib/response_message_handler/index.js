...
function escapeString(string) {
	const escapedString = string.replace(/\*/g, '星').replace(/\_/g, '\\_');
	...
	const message = _.map(teamDatas, teamData => {
		...
		_.forEach(teamData.teams, team => {
			_.forEach(team.characters, (character, index) => {
				...
				const formattedCharacterName = character.replace(/\(/g, ' ').replace(/\)/g, '')
				if (duplicatedCharacters.indexOf(characterName) !== -1) {
					team.characters[index] = `(${formattedCharacterName})`; // special highlight for duplicated characters
				} else {
					team.characters[index] = formattedCharacterName
				}
			});
		});
