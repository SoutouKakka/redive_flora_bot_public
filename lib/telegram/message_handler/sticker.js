...
async function Sticker(msg) {
  const {
    chat: { id: chatID },
    sticker: { file_unique_id: stickerID },
  } = msg;
  ...
  const stickerMapping = {
    ...
    AgADswIAAs8QCgY: { boss: 1, bossCycle: 31 },
    AgADtAIAAs8QCgY: { boss: 2, bossCycle: 31 },
    AgADtQIAAs8QCgY: { boss: 3, bossCycle: 31 },
    AgADtgIAAs8QCgY: { boss: 4, bossCycle: 31 },
    AgADtwIAAs8QCgY: { boss: 5, bossCycle: 31 },
  ...
  if (boss === "teams") {
    ...
    const { message } =
      responseMessageHandler.generateRecommendedTeam(teamData);
    ...
    // construct markdown message and respond to user
    const { message } =
      responseMessageHandler.generateMarkdownMessage(teamData);
    await bot.sendMessage(msg.chat.id, message, {
      parse_mode: "Markdown",
      disable_web_page_preview: true,
    });
