...
const checkResponseIsValid = (resString) => {
  return (
    !/#NAME/.test(resString) &&
    !/Loading/.test(resString) &&
    !/Unknown/.test(resString)
  );
};
...
async function getSheetData(spreadsheetID, range, retryCount = 0) {
  ...
  if (checkResponseIsValid(JSON.stringify(valueRanges))) {
    return valueRanges;
    ...
    return valueRanges.map((valueRange) => {
      const newValues = valueRange.values.map((value) =>
        value.map((v) => (checkResponseIsValid(v) ? v : ""))
      );
      return { ...valueRange, values: newValues };
    });
  }
}
